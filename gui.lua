...
local ConfigurationExtension = ".linear"
local LinearHubQuality = {}

local LinearHubLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default",
...
local LinearHub = game:GetObjects("rbxassetid://13853811008")[1]
LinearHub.Enabled = false
local spawn = task.spawn
local delay = task.delay

if game["Run Service"]:IsStudio() then
	function gethui() return LinearHub end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end
end

pcall(function()
	_G.LastRayField.Name = 'LinearHub'
...
local ParentObject = function(Gui)
	...
	_G.LastRayField = LinearHub
end
ParentObject(LinearHub)

local Camera = workspace.CurrentCamera
local Main = LinearHub.Main
...
local InfoPrompt = LinearHub.Info

LinearHub.DisplayOrder = 100
...
local BarType = 'Side'
local HoverTime = 0.3
local Notifications = LinearHub.Notifications

local SelectedTheme = LinearHubLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = LinearHub.Theme[ThemeName]
	for _, obj in ipairs(LinearHub:GetDescendants()) do
	...
	LinearHub.Main.BackgroundColor3 = SelectedTheme.Background
	LinearHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	LinearHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	LinearHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	LinearHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	LinearHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	LinearHub.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if LinearHubLibrary.Flags[FlagName] then
			spawn(function() 
				if LinearHubLibrary.Flags[FlagName].Type == "ColorPicker" then
					LinearHubLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if LinearHubLibrary.Flags[FlagName].CurrentValue or LinearHubLibrary.Flags[FlagName].CurrentKeybind or LinearHubLibrary.Flags[FlagName].CurrentOption or LinearHubLibrary.Flags[FlagName].Color ~= FlagValue then LinearHubLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(LinearHubLibrary.Flags) do
...
function LinearHubLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= LinearHubLibrary.Theme.Default then
		...
		if LinearHub.Name == "LinearHub" then
...
function Hide()
	...
	Debounce = true
	LinearHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 5})
...
function LinearHubLibrary:CreateWindow(Settings)
	LinearHub.Enabled = false
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Linear Hub | Gane Name"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "By Yoolovr / N3xKxp3r"
	if Settings.LoadingTitle ~= "Game Name" then
		LoadingFrame.Version.Text = "Linear Hub"
	end
	...
	pcall(function()
		...
		if not isfolder(LinearHubLibrary.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(LinearHubFolder.."/Discord Invites") then
			makefolder(LinearHubFolder.."/Discord Invites")
		end
		if not isfile(LinearHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then 
				writefile(LinearHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Linear Hub: Off")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(LinearHubFolder.."/Key System") then
			makefolder(LinearHubFolder.."/Key System")
		end
		...
		if isfile(LinearHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(LinearHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function(EnterPressed)
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(LinearHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
					end
				else
	...
	LinearHubQuality.Window = {Tabs = {}}
	local Window = LinearHubQuality.Window
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateSection(SectionName,Display)
			...
			if Display then
				...
				SectionValue.Holder.Parent = LinearHub.Holding
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						LinearHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					LinearHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					LinearHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					LinearHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					LinearHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function LinearHubLibrary:Destroy()
	LinearHub:Destroy()
end
...
function LinearHubLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				LinearHubLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
